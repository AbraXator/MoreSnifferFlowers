...
public class MSFClientConfig {
    ...
    public static int getBlockPatternRenderDistance() {
        if (!MSFClientConfig.CLIENT_CONFIG.isLoaded()) return 0;
        Minecraft minecraft = Minecraft.getInstance();
        int renderDistancePlayer = minecraft.options.getEffectiveRenderDistance();
        int configuredRenderDistance = MSFClientConfig.BLOCK_PATTERN_RENDER_DISTANCE.get();
