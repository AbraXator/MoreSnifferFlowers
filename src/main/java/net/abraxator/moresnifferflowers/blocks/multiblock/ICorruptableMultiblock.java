...
public interface ICorruptableMultiblock extends IMultiBlock {
    ...
    default void corruptionHelper(BlockState state, Level level, BlockPos pos, Entity entityInside){
        if(entityInside instanceof CorruptedProjectile corruptedProjectile && net.abraxator.moresnifferflowers.datagen.datamaps.Corruptable.canBeCorrupted(state.getBlock(), level.random)) {
    ...
    default void afterCorruption(BlockPos centrePos, Level level, BlockPos pos){
        if (!net.abraxator.moresnifferflowers.datagen.datamaps.Corruptable.canBeCorrupted(level.getBlockState(pos).getBlock(), level.random)) return;
